...
function createDelayedStream(chunks: any[], delayMs = 30): ReadableStream {
  return new ReadableStream({
    start(controller) {
      ...
      function next() {
        if (i < chunks.length) {
          controller.enqueue(chunks[i++]); // 将下一个数据块加入流中 
          setTimeout(next, delayMs); // 设置延迟，调用next继续处理
        } else {
          controller.close(); // 所有数据都处理完了，关闭流
        }
      }
      next(); // 开始第一次调用 next()
    },
